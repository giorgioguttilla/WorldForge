...
function rollingHillsPreset(): NoiseFieldGraphV1 {
  ...
  const fbm = node('rolling-fbm', 'fbm2d', 300, 105, { frequency: 0.0009, octaves: 5, gain: 0.52, seed: 71, rangeMin: 0, rangeMax: 45 });
...
function mountainsPreset(): NoiseFieldGraphV1 {
  ...
  const scale = node('mountains-scale', 'constFloat', 720, 245, { value: 220 });
  const lift = node('mountains-lift', 'multiply', 910, 135);
  const output = node('mountains-output', 'output', 1110, 135);
  return graph('preset-mountains', 'Mountains', 'Ridged peaks with broad elevation breakup.', [position, ridge, mask, multiply, power, scale, lift, output], [
    ...
    edge(power, 'value', lift, 'a'),
    edge(scale, 'value', lift, 'b'),
    edge(lift, 'value', output, 'value')
  ]);
}
...
function canyonPreset(): NoiseFieldGraphV1 {
  ...
  const scale = node('canyon-scale', 'constFloat', 505, 245, { value: -90 });
