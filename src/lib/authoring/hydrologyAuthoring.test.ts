...
const SIDE = 5;
const EMPTY: HydrologySceneV2 = { version: 2, waterBodies: [], riverSources: [], reaches: [] };
...
describe('authored graph hydrology tools', () => {
  ...
  it('stops at a baked basin and materializes its water body without tracing an exit', async () => {
...
function mockManager(options: {
  ...
  receivers?: number[];
  flows?: number[];
  basinRecords?: Array<{ id: number; fill: number; depth: number; bounds: [number, number, number, number] }>;
  ...
  const topology: HydrologyTopologyV3 = options.topology ?? {
    ...
    downstreamIds: new Uint32Array(records.length),
    spillCells: new Uint32Array(records.length),
    downstreamCells: new Uint32Array(records.length),
